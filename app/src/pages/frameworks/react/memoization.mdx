...
const Component = ({ users }) => {
  ...
  return <Component onClick={submit} />;
};
```

- Ключевой момент: ссылочная идентичность.
  Самое важное в понимании мемоизации - это работа со ссылками. При неизменных входных данных хук будет возращать всегда ту же самую ссылку:

```tsx
function ParentComponent() {
  const [count, setCount] = useState(0);

  // Без мемоизации - новая ссылка каждый рендер
  const regularObject = { data: "test" };

  // С мемоизацией - та же ссылка при неизменных зависимостях
  const memoizedObject = useMemo(() => ({ data: "test" }), []);
  ...
  return (
    <ChildComponent
      regularProp={regularObject} // Вызовет ререндер
      memoizedProp={memoizedObject} // Не вызовет ререндер
      onClick={handleClick} // Вызовет ререндер
      memoizedHandleClick={memoizedHandleClick} // Не вызовет ререндер
    />
  );
}
```
...
Для правильной работы memo следует передавать в качестве пропсов примитивные типы данных или мемоизировать имутабельные
...
- Мемоизация входящих аргументов для хука.

Всегда стоит обращать внимание на аргументы, которые передаёшь в кастомный хук, так как они могут быть в зависимостях у других хуков, что может привести к нарушению мемоизации.

```tsx
const args = React.useMemo(() => ({ paramId: id }), [id])
...
export function useTooltip(resId = 0, contentId, decoratorId, disabled, args, showDelay = 400 : UseTooltipParams) {
...
В данном случае нет смысла использовать useCallback, так как компонент Child будет перерендериваться при обновлении состояния или пропсов и сам компонент немемоизирован
...
Нет смысла оборачивать в useCallback и useMemo так как мы просто передаем в div
