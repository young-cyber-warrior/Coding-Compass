...
-   Если по каким-то причинам не удается определить данные внутри возвращаемого useMemo, имеет смысл дополнительно мемоизировать ссылочные типы данных, поскольку при их использовании вы будете обращаться к новой ссылке.
...
function App() {
    ...
    // При повторном рендере, если не мемоизировать previous и updateCount, будет создаваться новая ссылка.
